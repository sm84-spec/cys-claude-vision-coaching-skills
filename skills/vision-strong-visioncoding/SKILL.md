...
description: STRONG 직업흥미도 검사(STRONG Interest Inventory) 시뮬레이션 스킬. Holland의 RIASEC 6유형 모델 기반 — Realistic(현실형 R)·Investigative(탐구형 I)·Artistic(예술형 A)·Social(사회형 S)·Enterprising(진취형 E)·Conventional(관습형 C) 각 3문항씩 총 18문항을 자가 보고 형식으로 제시한다. 출제 방식은 *3문항씩 6라운드* — 사용자가 3문항에 응답하면 다음 3문항을 출제하는 순차 진행으로, 한 라운드에 6영역이 분산되어 패턴 답변을 방지한다. 응답 척도는 5점 — '전혀 관심 없음'(1점) / '별로 관심 없음'(2점) / '보통'(3점) / '관심 있음'(4점) / '매우 관심 있음'(5점). 18문항 완료 후 6영역별 점수(3~15점)를 막대 그래프로 시각화하고, 상위 3개 영역을 *Holland 트라이코드*(예: RIE·ASE·SCI)로 산출하여 적합 직업군을 추천한다. 본 검사는 시뮬레이션이며 *공식 STRONG 검사·전문 진로 상담을 대체하지 않음*을 명시한다. 사용자 입력 언어를 그대로 따라 응답한다 — 한국어 입력 시 한국어, English 입력 시 English, 中文 입력 시 中文. 사용자가 "STRONG 직업흥미도 검사", "직업 흥미 검사", "RIASEC 검사", "Holland 코드", "진로 적성 검사", "어떤 직업이 나에게 맞는가", "직업군 추천", "career interest test", "Holland Code"를 언급하거나, 진로·직업 탐색 맥락에서 흥미 유형 진단을 요청할 때 발동한다. 박사님 강의 청중·교회 청년부 진로 탐색·고등학생 진로 지도·성인 직업 전환자를 위해 설계되었다. vision 시리즈의 *진단 4종 세트*(readiness 능력·mbti 기질·values 가치·strong 직업 흥미) 마지막 모듈이다.
---
